function out_stats = h_transition_shocks_convex(init_ss,final_ss,transition_file,p_shocks,file_out,trans_x,file_suffix)
%
...
% no fixed cost (needed for code to run)
par_t0.C_f = 0;
...
for t=T-1:-1:1
    ...
    %%% set the grid
    gr = mainfuncMP_convex_fixed_C(par2,N,[],[],2);

    %%% solve the individual's problem by backward induction
    output  = mainfuncMP_convex_fixed_C(par2,N,gr,[],5,initval);
    pol     = output.pol;
    v       = output.vf;
    clear output
...
for t=1:T-1
    ...
    %%% recreate grid
    gr_now = mainfuncMP_convex_fixed_C(par2,N,[],[],2);
    ...
    initval.g0 = g0(:);
    initval.trmat = [];
    output2 = mainfuncMP_convex_fixed_C(par2,N,gr_now,pol_t{t},6,initval);
    g0  = output2.g1(:);
 ...
 % simulation parameters
    ...
    for t=2:T_sim
        ...
        if t>=t_burn
            ...
            % price / agg C along transition path
            Px = P_t(tnew);
            Yd = 1/(P_t_input(tnew)*par_t0.chi);
            
            %%% Replace policies with transition policies
            % continuation kprime policy
            FK_cont = griddedInterpolant({1:N.s,gr.k_grid},pol_t{tnew}.kpr,'linear');
            % exit probs, conditioned on state (s,k)
            Fexit = griddedInterpolant({1:N.s,gr.k_grid},pol_t{tnew}.pr_stay,'linear');
            % entry probs, conditioned on state (s)
            Fentry = griddedInterpolant({1:N.s},pol_t{tnew}.pr_fstar_enter,'linear');
            % entry kprime policy
            FK_entry = griddedInterpolant({1:N.s},pol_t{tnew}.kpr_up,'linear');
            % exit choice, conditioned on state (s,k)
            Fstar_exit = griddedInterpolant({1:N.s,gr.k_grid},pol_t{tnew}.fstar,'linear');
            % entry choice, conditioned on state (s)
            Fstar_enter = griddedInterpolant({1:N.s},pol_t{tnew}.f_star_enter,'linear');
