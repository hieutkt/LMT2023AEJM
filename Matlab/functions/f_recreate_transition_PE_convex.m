function f_recreate_transition_PE_convex(init_ss,final_ss,file_in,file_out,trans_x,file_suffix,exovars)
%
% Feed in desired price price to recreate solution
...
load([file_in init_ss], 'pol', 'Px', 'par2', 'gr', 'N', 'Pnow', 'old_C')
...
clear pol Px Pnow par gr new_C
...
for t=1:T-1
    ...
    if t==1
        pol_01 = pol_0;
        pol_01.kpr = pol_t{t}.kpr;
        output2 = mainfuncMP_convex(par2,N,gr_now,pol_01,6,initval);
        g0_01  = output2.g1(:);

        pol_02 = pol_0;
        pol_02.pr_fstar_enter = pol_t{t}.pr_fstar_enter;
        output2 = mainfuncMP_convex(par2,N,gr_now,pol_02,6,initval);
        g0_02  = output2.g1(:);

        pol_03 = pol_0;
        pol_03.pr_stay = pol_t{t}.pr_stay;
        output2 = mainfuncMP_convex(par2,N,gr_now,pol_03,6,initval);
        g0_03  = output2.g1(:);

        pol_04 = pol_0;
        pol_04.kpr_up = pol_t{t}.kpr_up;
        output2 = mainfuncMP_convex(par2,N,gr_now,pol_04,6,initval);
        g0_04  = output2.g1(:);

        pol_05 = pol_0;
        pol_05.kpr = pol_t{t}.kpr;
        pol_05.pr_stay = pol_t{t}.pr_stay;
        output2 = mainfuncMP_convex(par2,N,gr_now,pol_05,6,initval);
        g0_05  = output2.g1(:);

        pol_06 = pol_0;
        pol_06.kpr = pol_t{t}.kpr;
        pol_06.kpr_up = pol_t{t}.kpr_up;
        output2 = mainfuncMP_convex(par2,N,gr_now,pol_06,6,initval);
        g0_06  = output2.g1(:);
    end
    if t==2
        % inner integral for domestic goods
        inner_Cd = gr_now.y_i(:).^((par2.epsi-1)/par2.epsi)'*g0_01(:);
        % Domestic real output
        Cd = inner_Cd^(par2.epsi/(par2.epsi-1));
        Kd = gr_now.k_xgrid(:)'*g0_01(:);  % total capital stock of domestic firms
        Ld = gr_now.lstar(:)'*g0_01(:);    % total labor of domestic firms
        % Measured corrected TFP
        M_norm = sum(g0_01(:)).^(1/(par2.epsi-1));
        TFPQ_counterfactual_1 = Cd./(M_norm.*(Kd.^par2.alph .* Ld.^(1-par2.alph)));

        % inner integral for domestic goods
        inner_Cd = gr_now.y_i(:).^((par2.epsi-1)/par2.epsi)'*g0_02(:);
        % Domestic real output
        Cd = inner_Cd^(par2.epsi/(par2.epsi-1));
        Kd = gr_now.k_xgrid(:)'*g0_02(:);  % total capital stock of domestic firms
        Ld = gr_now.lstar(:)'*g0_02(:);    % total labor of domestic firms
        % Measured corrected TFP
        M_norm = sum(g0_02(:)).^(1/(par2.epsi-1));
        TFPQ_counterfactual_2 = Cd./(M_norm.*(Kd.^par2.alph .* Ld.^(1-par2.alph)));

        % inner integral for domestic goods
        inner_Cd = gr_now.y_i(:).^((par2.epsi-1)/par2.epsi)'*g0_03(:);
        % Domestic real output
        Cd = inner_Cd^(par2.epsi/(par2.epsi-1));
        Kd = gr_now.k_xgrid(:)'*g0_03(:);  % total capital stock of domestic firms
        Ld = gr_now.lstar(:)'*g0_03(:);    % total labor of domestic firms
        % Measured corrected TFP
        M_norm = sum(g0_03(:)).^(1/(par2.epsi-1));
        TFPQ_counterfactual_3 = Cd./(M_norm.*(Kd.^par2.alph .* Ld.^(1-par2.alph)));

        % inner integral for domestic goods
        inner_Cd = gr_now.y_i(:).^((par2.epsi-1)/par2.epsi)'*g0_04(:);
        % Domestic real output
        Cd = inner_Cd^(par2.epsi/(par2.epsi-1));
        Kd = gr_now.k_xgrid(:)'*g0_04(:);  % total capital stock of domestic firms
        Ld = gr_now.lstar(:)'*g0_04(:);    % total labor of domestic firms
        % Measured corrected TFP
        M_norm = sum(g0_04(:)).^(1/(par2.epsi-1));
        TFPQ_counterfactual_4 = Cd./(M_norm.*(Kd.^par2.alph .* Ld.^(1-par2.alph)));

        % inner integral for domestic goods
        inner_Cd = gr_now.y_i(:).^((par2.epsi-1)/par2.epsi)'*g0_05(:);
        % Domestic real output
        Cd = inner_Cd^(par2.epsi/(par2.epsi-1));
        Kd = gr_now.k_xgrid(:)'*g0_05(:);  % total capital stock of domestic firms
        Ld = gr_now.lstar(:)'*g0_05(:);    % total labor of domestic firms
        % Measured corrected TFP
        M_norm = sum(g0_05(:)).^(1/(par2.epsi-1));
        TFPQ_counterfactual_5 = Cd./(M_norm.*(Kd.^par2.alph .* Ld.^(1-par2.alph)));

        % inner integral for domestic goods
        inner_Cd = gr_now.y_i(:).^((par2.epsi-1)/par2.epsi)'*g0_06(:);
        % Domestic real output
        Cd = inner_Cd^(par2.epsi/(par2.epsi-1));
        Kd = gr_now.k_xgrid(:)'*g0_06(:);  % total capital stock of domestic firms
        Ld = gr_now.lstar(:)'*g0_06(:);    % total labor of domestic firms
        % Measured corrected TFP
        M_norm = sum(g0_06(:)).^(1/(par2.epsi-1));
        TFPQ_counterfactual_6 = Cd./(M_norm.*(Kd.^par2.alph .* Ld.^(1-par2.alph)));
        
        % MPK dispersion for counterfactual 6
        mpk_grid = log(gr_now.py_i) - log(gr_now.k_xgrid) + log(par2.alpha_k);
        mean_mpk = mpk_grid(:)'*g0_06(:)/sum(g0_06(:));
        sd_mpk_6 = sqrt((mpk_grid(:)'-mean_mpk).^2 * g0_06(:)/sum(g0_06(:)));

    end

end

TFPQ_counterfactual_1_t = TFPQ_t;
TFPQ_counterfactual_2_t = TFPQ_t;
TFPQ_counterfactual_3_t = TFPQ_t;
TFPQ_counterfactual_4_t = TFPQ_t;
TFPQ_counterfactual_5_t = TFPQ_t;
TFPQ_counterfactual_6_t = TFPQ_t;
sd_mpk_6_t              = sd_mpk_t;
TFPQ_counterfactual_1_t(2) = TFPQ_counterfactual_1;
TFPQ_counterfactual_2_t(2) = TFPQ_counterfactual_2;
TFPQ_counterfactual_3_t(2) = TFPQ_counterfactual_3;
TFPQ_counterfactual_4_t(2) = TFPQ_counterfactual_4;
TFPQ_counterfactual_5_t(2) = TFPQ_counterfactual_5;
TFPQ_counterfactual_6_t(2) = TFPQ_counterfactual_6;
sd_mpk_6_t(2)              = sd_mpk_6;
...
if par2.lam>0
    save([file_out '/transition_recreate' file_suffix],'*_t', 'T', '*_autarky', '*_t0', 'par*','-v7.3')
else
    save([file_out '/transition_recreate_q1' file_suffix],'*_t', 'T', '*_autarky', '*_t0', 'par*','-v7.3')
end
