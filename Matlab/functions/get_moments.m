function output = get_moments(par,N,gr,Px,pol,sim_sw,dat_out_sw)
...
s_lamb_0 = lamb_stay(i_mpk_raw_s);
% compute cdf
cs_s_lamb_0 = cumsum(s_lamb_0);
...
for iq = 1:nq
    ...
    % locate which part of rw today corresponds to q (using full dist of
    % today)
    lq = (iq-1)/nq;
    uq = (iq/nq)*(iq<nq) + 100*(iq==nq);
    i_q = cs_s_lamb_0>=lq & cs_s_lamb_0<uq;
    i_q_unsort = i_q(i_inv_sort_mpk_raw);

    % store average r in this quantile
    temp_r = mpk_raw_s(i_q);
    temp_lamb = s_lamb_0(i_q);
...
TFPQ = C./(M_norm.*(Kd.^par.alph .* Ld.^(1-par.alph)));
...
window_010 = 0.10;
window_020 = 0.20;
window_050 = 0.49; % 0.2*sd(LMT)/sd(CH)
...
if sim_sw==1 && output.avg.exit_rate>0.01 % do only if exit rate >1%
    ...
    FK_cont = griddedInterpolant({1:N.s,1:N.k},pol.i_kpr,'nearest');
    % exit probs, conditioned on state (s,k)
    Fexit = griddedInterpolant({1:N.s,1:N.k},pol.pr_stay,'nearest');
    % entry probs, conditioned on state (s)
    Fentry = griddedInterpolant({1:N.s},pol.pr_fstar_enter,'nearest');
    % entry kprime policy
    FK_entry = griddedInterpolant({1:N.s},pol.i_kpr_up,'nearest');
    % exit choice, conditioned on state (s,k)
    Fstar_exit = griddedInterpolant({1:N.s,1:N.k},pol.fstar,'nearest');
    % entry choice, conditioned on state (s)
    Fstar_enter = griddedInterpolant({1:N.s},pol.f_star_enter,'nearest');
...
%     %%%
    ...
    i_k_active_sim = i_k_active_sim(t_burn:end);
    ...
    %%% transition matrix
    nq = 3;
    logmpk_stay_sim_qvec = zeros(nq+1,T2);
    logmpk_stay_prime_sim_qvec = zeros(nq+1,T2);
    logmpk_sim_qmat = cell(T2,1);
    logmpk_prime_sim_qmat = cell(T2,1);
    mpk_now = cell(T2-1,1);
    mpk_tmr = cell(T2-1,1);
    for t=1:T2-1
        
        % directly pull out py from grid
        i_pyi = sub2ind([N.s,N.k],i_s_active_sim{t}(:),i_k_active_sim{t}(:));
        
        % compute MPK for every active firm
        logmpk_sim = mpk_grid(i_pyi);
        ...
        i_kcont = i_kcont_sim{t}; %(1:sum(i_stay_sim{t}));
        i_scont = i_scont_sim{t}; %(1:sum(i_stay_sim{t}));
        
        % directly pull out py from grid
        i_pyi = sub2ind([N.s,N.k],i_scont(:),i_kcont(:));
        
        % compute MPK for every active firm
        logmpk_prime_sim = mpk_grid(i_pyi);
    ...
    output.ik_regs = ik_elas(nq,i_mpk_now,mpk_now,i_kpr_vec,i_k_active_sim,i_s_active_sim,i_s_active_enter_sim,i_stay_sim,gr,par,T2);
...
function outregs = ik_elas(nq,i_mpk_now,mpk_now,i_kpr_vec,i_k_active_sim,i_s_active_sim,i_s_active_enter_sim,i_stay_sim,gr,par,T2)
    ...
    i_kpr_vec_2 = i_kpr_vec(1:length(i_mpk_now));

    % pull out current k for same period
    i_k_active_vec = [];
    for tt = 1:T2-1
        i_k_active_temp = i_k_active_sim{tt}(i_stay_sim{tt});
        i_k_active_vec = [i_k_active_vec ; i_k_active_temp(:)];
    end
    % log k
    logk = log(gr.k_grid(i_k_active_vec));
        % compute growth rate of capital, winsorized
    log_grk =  log(gr.k_grid(i_kpr_vec_2)./gr.k_grid(i_k_active_vec));
    ...
    ik = gr.k_grid(i_kpr_vec_2)./gr.k_grid(i_k_active_vec) - (1-par.delta);
