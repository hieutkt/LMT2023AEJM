function f_transition_mkt(init_ss,final_ss,file_out,trans_x,file_suffix)
%
% Solution method
...
%     if iter == 1
        for t=T-1:-1:1
            ...
            if par2.lam==0 && par2.zet==0
                ...
                output  = mainfuncMP(par2,N,gr,[],5,initval);
            end
            pol     = output.pol;
            v       = output.vf;
            clear output
...
%         P_t_new = sort(P_t_new);
        ...
        initval.trmat = [];
        if par2.lam==0 && par2.zet==0
            output2 = mainfuncMP_q1(par2,N,gr_t{t},pol_t{t},6,initval);
        else
            output2 = mainfuncMP(par2,N,gr_t{t},pol_t{t},6,initval);
        end
        g0  = output2.g1(:);
        clear initval
    end
...
for t=1:T-1
    ...
    if par2.lam==0 && par2.zet==0
        ...
        output2 = mainfuncMP(par2,N,gr_now,pol_t{t},6,initval);
    end
    g0  = output2.g1(:);
...
function xcons = mkt_static(P_now,par_in,N,Pf,dM,g0)
    ...
    % set the grid
    if par_in.lam==0 && par_in.zet==0
        gr_now = mainfuncMP_q1(par_in,N,[],[],2);
    else
        gr_now = mainfuncMP(par_in,N,[],[],2);
    end
...
function [xcons,v,pol,gr_now] = mkt_static_val(P_now,par_in,N,Pf,dM,g0,initval)
    ...
    % solve the individual's problem by backward induction
    if par2.lam==0 && par2.zet==0
        output  = mainfuncMP_q1(par_in,N,gr_now,[],5,initval);
    else
        output  = mainfuncMP(par_in,N,gr_now,[],5,initval);
    end
    pol     = output.pol;
    v       = output.vf;
    clear output
