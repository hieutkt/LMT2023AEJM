function output = mainfuncMP_fixed_C(par,N,grids,pol,sw,initval)
% This version: Baseline. Allows for C_f, but no convex cost.
%
% This version PE: Exactly the same as "GE" for interest rates.
% Difference is "Y" is fixed to time-0.
    ...
    while diff_v > tol_v && iter_v < maxiter_v
        ...
        if lam~=0
            ...
            for is=1:N.s
                ...
                for ikpr = last_kpr:N.k
                    ...
                    % compute candidate value
                    v_candidate = -Q*(1-lam)*k_grid(ikpr) + betaa*Ev(is,ikpr);
            ...
            if C_f==0
                % if only irr, just need consistency checks
                ...
                % (v_stay(s,k))
                v_stay = y + i_up.*Q*(1-delta).*k_xgrid + i_down.*Q*(1-lam)*(1-delta).*k_xgrid + v_cont;
                
            elseif C_f>0
                %%% fixed cost version: check optimality 
                
                switch model_fixed_cost
                    % add in value of installed k and subtract out fixed cost
                    case('asker')
                        v_up_cont = v_up_cont + Q*(1-delta).*k_xgrid - C_f*y;
                        v_down_cont = v_down_cont + Q*(1-lam)*(1-delta).*k_xgrid - C_f*y;
                    case('ch')
                        v_up_cont = v_up_cont + Q*(1-delta).*k_xgrid - C_f*Q*k_xgrid;
                        v_down_cont = v_down_cont + Q*(1-lam)*(1-delta).*k_xgrid - C_f*Q*k_xgrid;
                end
                
                % use consistency check to set irrelevant ss to 0
                v_up_cont = v_up_cont.*i_up;
                v_down_cont = v_down_cont.*i_down;
                
                % recompute inaction values
                v_inact_cont = betaa.*Ev(i_inact_s);
                
                % reset indicators where optimality satified
                i_up = (v_inact_cont<v_up_cont) & (v_up_cont>v_down_cont);
                i_down = (v_inact_cont<v_down_cont) & (v_up_cont<v_down_cont);
                I_inact = ~i_up & ~i_down;

                % use optimality check to set irrelevant ss to 0
                v_up_cont = i_up.*v_up_cont;
                v_down_cont = i_down.*v_down_cont;
                v_inact_cont = I_inact.*v_inact_cont;   
        ...
        if ~isnan(mu_fstay_b)
            ...
            if C_f==0
                ...
                switch model_fixed_cost
                    case('asker')
                        v_exit = y - C_f*y + hk(k_xgrid)*Q*(1-delta).*k_xgrid;
                    case('ch')
                        v_exit = y - C_f*Q*k_xgrid + hk(k_xgrid)*Q*(1-delta).*k_xgrid;
                end
            end
    ...
    if lam~=0
        ...
        for is=1:N.s
            ...
            for ikpr = last_kpr:N.k
                ...
                % compute candidate value
                v_candidate = -Q*k_grid(ikpr) + betaa_pr*Ev(is,ikpr);
%                 v_candidate = -Q*k_grid(ikpr)*P + betaa*(P_pr)*Ev(is,ikpr);
        ...
        for is=1:N.s
            ...
            for ikpr = last_kpr:N.k
                ...
                % compute candidate value
                v_candidate = -Q*(1-lam)*k_grid(ikpr) + betaa_pr*Ev(is,ikpr);
        ...
        % inaction
        I_inact = ~i_up & ~i_down;
        v_inact_cont = squeeze(I_inact).*betaa_pr.*Ev(i_inact_s);
