%% model_trade_shock_frictionless.m
%
...
function model_trade_shock_frictionless_serial(mat_out)
clc
close all

disp('solving frictionless multi-industry model')
...
for ii=1:N_ind
    ...
    disp(['Running industry ' num2str(ii)])
    
    % generate stats
    out_stats = h_transition_shocks('ss_autarky_q1_baseline','ss_tradeshock_q1_baseline','transition_q1_baseline',imp_shocks_draw(:,ii),mat_out,t_trans);
...
load([mat_out '/transition_q1_baseline'],'P_Cd_t','P_Cf_t')
...
clear temp_cell temp_cell2

save([mat_out '/results/stats_frictionless_data'],'stats_micro','stats_agg');
...
save([mat_out '/results/regs_frictionless'],'bhat_out','ci_out')
